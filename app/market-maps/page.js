...
// flat list for the comparison bar chart, sorted by rent growth desc
const ALL = MARKET_GROUPS.flatMap((g) => g.markets).sort(
...
function MarketMapsInner() {
  ...
  return (
    <div className="pt-12 pb-10">
      ...
      <p className="mt-3 max-w-2xl text-muted">Multifamily fundamentals scored and ranked across top U.S. metros.</p>

      <div className="card mt-8 p-6">
        <h2 className="mb-4 font-semibold text-ink">Rent Growth Comparison — Top Markets</h2>
        <BarChart />
      </div>
          ...
          <div className="overflow-hidden rounded-lg border border-[var(--line)]">
            {g.markets.map((m, i) => (
              <div key={m.city} className={`grid grid-cols-2 items-center gap-4 px-6 py-5 md:grid-cols-[1.4fr_repeat(4,1fr)_auto] bg-bg2 ${i ? "border-t border-[var(--line)]" : ""}`}>
                ...
                <Metric label="Rent Growth" value={m.rent} tone={m.tone} />
                <Metric label="Vacancy" value={m.vac} />
                <Metric label="Cap Rate" value={m.cap} />
                <Metric label="NOI Growth" value={m.noi} tone={m.tone} />
