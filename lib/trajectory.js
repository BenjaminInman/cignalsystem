...
export function trajectory(yoySeries, { goodUp = true, window = TRAJ_WINDOW } = {}) {
  ...
  for (let end = win.length; end > 0; end -= SEGMENT) {
    ...
    segments.unshift({
      ...
      // Segment MEAN is what the UI should display. Endpoints are two single months
      // and on a noisy series they can fall while the fitted slope rises — which
      // makes the label look like it contradicts the numbers beside it. The mean
      // uses every point in the window and moves with the slope.
      mean: round(chunk.reduce((a, b) => a + b, 0) / chunk.length),
