...
export async function getTicker() {
  ...
  const ar = await series("apt_rent_estimate", 14);
  if (ar && ar.length >= 13) {
    const yoy = (ar[0] / ar[12] - 1) * 100;
    const mom = ((ar[0] - ar[1]) / ar[1]) * 100;
    items["AVG RENT GROWTH"] = { value: `${yoy >= 0 ? "+" : ""}${yoy.toFixed(1)}% YoY`, delta: `${mom >= 0 ? "+" : ""}${mom.toFixed(1)}%`, dir: yoy >= 0 ? "up" : "down", live: true };
  }
  ...
  if (av && av.length >= 2) {
    const dpp = av[0] - av[1];
    items["NATIONAL VACANCY"] = { value: `${av[0].toFixed(1)}%`, delta: `${dpp >= 0 ? "+" : ""}${dpp.toFixed(1)}pp`, dir: dpp <= 0 ? "up" : "down", live: true };
  }
  ...
  if (tom && tom.length >= 2) {
    const dd = tom[0] - tom[1];
    items["DAYS ON MARKET"] = { value: `${Math.round(tom[0])} days`, delta: `${dd >= 0 ? "+" : ""}${dd.toFixed(1)}`, dir: dd <= 0 ? "up" : "down", live: true };
  }
  ...
  if (cr && cr.length >= 1) {
    const bps = cr.length >= 2 ? Math.round((cr[0] - cr[1]) * 100) : null;
    items["CAP RATE (NATAVG)"] = {
      value: `${cr[0].toFixed(2)}%`,
      delta: bps == null ? "" : `${bps >= 0 ? "+" : ""}${bps}bps`,
