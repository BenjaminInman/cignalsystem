...
export async function getTicker() {
  ...
  // Apartment List time on market (apt_time_on_market, days) → APT DAYS TO LEASE
  // Labelled explicitly: this is apartments, NOT the Realtor.com for-sale
  // days-on-market series shown on the Indicators tab. Different markets.
  // delta MoM days; fewer days = tightening = bullish
  const tom = await series("apt_time_on_market", 2);
  if (tom && tom.length >= 2) {
    const dd = tom[0].value - tom[1].value;
    items["APT DAYS TO LEASE"] = { value: `${Math.round(tom[0].value)} days`, delta: signedFixed(dd, 1), dir: dd <= 0 ? "up" : "down", live: true };
  }
  ...
  // Absorption (Census SOMA, quarterly %) → ABSORPTION RATE.
  // Uses the SAME 4-quarter average the Indicators tab reports, so the two
  // surfaces can never disagree about the same named metric. Previously this
  // row had no override at all and rendered a hard-coded 92.4% sample under the
  // LIVE FEED banner while the real series read ~46.8% — a fabricated number
  // presented as market data.
  const ab = await series("absorption", 8);
  if (ab && ab.length >= 4) {
    const t4 = ab.slice(0, 4).reduce((s, r) => s + r.value, 0) / 4;
    const prev4 =
      ab.length >= 5 ? ab.slice(1, 5).reduce((s, r) => s + r.value, 0) / 4 : null;
    const d = prev4 == null ? null : t4 - prev4;
    items["ABSORPTION RATE"] = {
      value: `${t4.toFixed(1)}%`,
      delta: d == null ? "4-qtr avg" : signedFixed(d, 1, "pp"),
      dir: d == null ? "up" : d >= 0 ? "up" : "down",
      live: true,
    };
  }

  // Cap rate (national avg) — manually maintained monthly indicator in the DB.
  // Always read the value from the DB so a manual update propagates here, but
  // only claim `live` once it is a real series (>=2 points, so there is a delta
  // and a pipeline behind it). With a single hand-entered observation the value
  // still shows, without the live dot — it is a maintained figure, not a feed.
  // Never fall through to the static sample: that would pin the ticker to a
  // stale number even after the DB is updated.
  const cr = await series("cap_rate", 2);
  if (cr && cr.length >= 1) {
    const bps = cr.length >= 2 ? Math.round((cr[0].value - cr[1].value) * 100) : null;
    items["CAP RATE (NATAVG)"] = {
      value: `${cr[0].value.toFixed(2)}%`,
      delta: bps == null ? "manual" : `${bps >= 0 ? "+" : ""}${bps}bps`,
      dir: bps == null ? "down" : bps <= 0 ? "up" : "down",
      live: cr.length >= 2,
    };
  }
