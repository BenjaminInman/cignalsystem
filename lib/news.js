...
// Primary feed URL per source, grouped by vertical. If a source ever stops
// returning items, swap its URL here — the page falls back gracefully.
const FEEDS = {
  multifamily: [
  ...
  "general-real-estate": [
    { source: "The Real Deal", url: "https://therealdeal.com/feed/" },
    { source: "HousingWire", url: "https://www.housingwire.com/feed/" },
    { source: "Inman", url: "https://www.inman.com/feed/" },
    { source: "Commercial Observer", url: "https://commercialobserver.com/feed/" },
  ],
};

function feedsFor(slug) {
  return FEEDS[slug] || FEEDS.multifamily;
}
...
export async function getNews(limit = 18, slug = "multifamily") {
  const results = await Promise.allSettled(feedsFor(slug).map(fetchFeed));
