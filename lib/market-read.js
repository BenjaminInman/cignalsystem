...
// Level-style, lower-is-better (vacancy, unemployment).
function scoreStress(r, level) {
  ...
  let sentiment;
  if (falling) sentiment = "g";
  else if (rising) sentiment = "r";
  else sentiment = "a";
  return { phase, sentiment };
}
...
const KINDS = {
  ...
  wages: { read: growthReading, score: scoreGrowth },
  pce: { read: growthReading, score: scoreGrowth },
  vacancy: { read: levelReading, score: (r) => scoreStress(r, LEVELS.vacancy) },
  unemployment: { read: levelReading, score: (r) => scoreStress(r, LEVELS.unemployment) },
