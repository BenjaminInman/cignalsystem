...
const WHEEL = {
  ...
  // Single-family: the owner-occupied and SFR-rental housing cycle. Uses the
  // ONE-UNIT supply series (PERMIT1 / HOUST1F / COMPU1USA) rather than the
  // all-units totals, so multifamily construction cannot flatter or depress the
  // single-family read. Rent is the Zillow SFR-only cut -- national SFR rent is
  // running +2.9% YoY against multifamily's +1.3%, which is exactly the signal
  // a blended series would hide.
  //
  // existing_home_sales is deliberately ABSENT. It is NAR data and FRED carries
  // only a rolling 13 months, so it cannot support a YoY-plus-z-score gauge.
  // Display-only, same posture as hy_oas.
  "single-family": [
    { slug: "permits_1unit", label: "Permits", role: "leading", type: "growth" },
    ...
    { slug: "sf_starts", label: "Starts", role: "leading", type: "growth" },
    { slug: "sf_completions", label: "Completions", role: "coincident", type: "growth" },
    { slug: "employment", label: "Jobs", role: "coincident", type: "growth" },
    { slug: "gdp", label: "GDP", role: "coincident", type: "gdp" },
    { slug: "case_shiller", label: "Home Prices", role: "trailing", type: "growth" },
    { slug: "zori_national_sfr", label: "Rent", role: "trailing", type: "growth" },
...
const UNIVERSE = {
  ...
  "single-family": {
    leading: [
      "permits_1unit", "sf_starts", "new_home_sales", "months_supply",
    ...
    lagging: [
      "case_shiller", "median_sale_price", "zhvi_national", "zori_national_sfr",
      "homeownership_rate", "sf_completions", "mortgage_delinquency",
      "cpi_shelter", "gdp", "real_pce", "unemployment",
    ],
  },
  "general-real-estate": {
    leading: [
...
const DOMAIN = {
  ...
  "single-family": "the U.S. single-family housing market — owner-occupied and single-family rental",
...
const RENT_METRO = {
  ...
  "single-family": "zori_metro_sfr",
  "general-real-estate": "zori_metro",
};
const RENT_NATIONAL = {
  multifamily: "zori_national_mf",
  "single-family": "zori_national_sfr",
