...
export async function getLiveIndicators() {
  ...
  const [ahe, pay, gdp, pce, umc, cpi, ff, dom, perm, vac, rent, ys, ic, cl, cap, abs, dsc, mig, fc, e2024, e2534, rd, cre] = await Promise.all([
  ...
  if (rr) {
    ...
    out["Effective Rent Growth"] = { value: pct(rr.latest), unit: "ZORI, YoY", change: `${signed(d)}% MoM`, note: "Zillow Observed Rent Index — national market asking rents", tone: "neutral", trend: rr.trend, periods: rr.periods, live: true };
  }
