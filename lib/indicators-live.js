...
export async function getLiveIndicators() {
  ...
  if (abs && abs.length >= 1) {
    ...
    out["Net Absorption Rate"] = { value: `${abs[0].value.toFixed(1)}%`, unit: "leased in 3 mo", change: d == null ? "new" : `${signed(d, 1)}pp QoQ`, note: "New apartments leased within 3 months of completion — Census SOMA, quarterly", tone: d == null ? "neutral" : toneFor(d, true), trend, periods, live: true };
  }
  if (dsc && dsc.length >= 1) {
