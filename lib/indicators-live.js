...
export async function getLiveIndicators() {
  ...
  const [ahe, aher, pay, gdp, pce, umc, cpi, ff, dom, perm, vac, rent, ys, ic, cl, cap, un, abs, zrd, dsc, mig, fc, e2024, e2534, rd, cre, rentmf, hvn, t2y, baa, hyo, sfr, mls, jo, jq, aim, aimr, aimv, ehs, rb, mgr, mhi, mhv, ro, ou] = await Promise.all([
  ...
  if (ehs && ehs.length >= 2) {
    const d = ehs[0].value - ehs[1].value;
    const { trend, periods } = lastN(ehs, 9, 2);
    out["Existing Home Sales"] = { value: `${ehs[0].value.toFixed(2)}M`, unit: "annualized", change: `${signed(d, 2)}M MoM`, note: "For-sale market throughput", tone: toneFor(d, true), trend, periods, live: true };
  }
