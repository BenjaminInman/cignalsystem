...
export async function getLiveIndicators() {
  ...
  const [ahe, pay, gdp, pce, umc, cpi, ff, dom, perm, vac, rent, ys, ic, cl, cap, abs, zrd, dsc, mig, fc, e2024, e2534, rd, cre, rentmf, hvn] = await Promise.all([
  ...
  if (abs && abs.length >= 1) {
    ...
    out["Absorption Rate (3-Mo)"] = { value: `${abs[0].value.toFixed(1)}%`, unit: "leased in 3 mo", change: d == null ? "new" : `${signed(d, 1)}pp QoQ`, note: "New apartments leased within 3 months of completion — Census SOMA, quarterly", tone: d == null ? "neutral" : toneFor(d, true), trend, periods, live: true };
  }
  if (zrd && zrd.length >= 13) {
    // Unitless engagement index: the honest read is YoY change, not the level.
    const yoy = zrd[0].value - zrd[12].value;
    const prevYoy = zrd.length >= 14 ? zrd[1].value - zrd[13].value : null;
    const d = prevYoy == null ? null : yoy - prevYoy;
    ...
    out["Renter Demand Index (ZORDI)"] = {
      ...
      note: "Zillow renter engagement — demand intent, leads leasing. Directional index; read the trend, not the level.",
