...
export async function getLiveIndicators() {
  ...
  const [ahe, pay, gdp, pce, umc, cpi, ff, dom, perm, vac, rent, ys, ic, cl, cap, abs, dsc, mig, fc, e2024, e2534, rd, cre, ev] = await Promise.all([
  ...
  if (ev && ev.length >= 1) {
    const d = ev.length >= 2 ? ev[0].value - ev[1].value : null;
    const yr = new Date(ev[0].date).getUTCFullYear();
    const recent = ev.slice(0, 9).reverse();
    out["Average Eviction Rate"] = { value: `${ev[0].value.toFixed(2)}%`, unit: `of renters (${yr})`, change: d == null ? "Eviction Lab" : `${signed(d, 2)}pp YoY`, note: "Physical (court-ordered) evictions — Eviction Lab, through 2018", tone: d == null ? "bear" : toneFor(d, false), trend: recent.map((o) => o.value), periods: recent.map((o) => String(new Date(o.date).getUTCFullYear())), live: true };
  }
