...
export async function getLiveIndicators() {
  ...
  // U-6 bands are anchored on the series' own 1994-2019 distribution rather
  // than invented: that window's median sits near 9%, with sub-8% readings
  // confined to the late-90s and 2018-19 peaks of tightness and 9.5%+ marking
  // genuine slack. Deliberately NOT the 2.5-4.5% U-3 band -- U-6 runs roughly
  // twice U-3 by construction, and reusing the headline band here would paint
  // every ordinary month red.
  if (u6 && u6.length >= 13 && yearAgo(u6)) {
    const v = u6[0].value;
    const yoy = yoyAbs(u6);
    const tone = v > 9.5 ? "bear" : v >= 8 ? "neutral" : "bull";
    const { trend, periods } = lastN(u6, 9, 1);
    out["Underemployment Rate (U-6)"] = {
      value: `${v.toFixed(1)}%`, unit: "U-6, SA",
      change: `${signed(yoy, 1)}pp YoY`,
      note: v > 9.5 ? "Elevated slack beneath the headline"
          : v >= 8 ? "Ordinary range for this measure"
          : "Unusually little slack",
      tone, trend, periods, live: true,
    };
  }

  // The gap is scored on DIRECTION, not level. Its level drifts with the size
  // of the involuntary part-time workforce -- a structural quantity -- while the
