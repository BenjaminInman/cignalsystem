...
const LABEL = {
  ...
  days_on_market: "Days on Market", absorption: "Net Absorption", emp_2024: "Employment 20-24",
...
export async function nationalIntel() {
  const meta = await sb(
    `indicators?select=slug,classification,higher_is_better&classification=in.(leading,trailing)`
  );
  if (!meta) return null;
  const metaBy = Object.fromEntries(meta.map((m) => [m.slug, m]));
