...
export async function nationalIntel(vertical) {
  ...
  // Panel tier per slug (DIRECT measures MF, DRIVER moves it, MACRO backdrop).
  // Drives the home "Multifamily Trendline" card ordering. Unlisted slugs read
  // as MACRO so a new indicator never jumps the queue without a deliberate
  // panel_tiers row.
  const tierRows = (await sb("panel_tiers?select=slug,tier")) || [];
  const tierOf = Object.fromEntries(tierRows.map((r) => [r.slug, r.tier]));
  ...
  // Each event carries `spark` (trailing z-history vs its own trend baseline)
  // and `tier` from panel_tiers. `mf` stays true for DIRECT|DRIVER so existing
  // consumers keep working, but the home section orders by tier.
  const events = [];
  for (const slug of slugs) {
