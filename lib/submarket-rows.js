...
export function metroSignalRows(sig) {
  ...
  if (s.employment) {
    ...
    // Headline is the COUNT of jobs added over the past 12 months, not the
    // percentage. Payroll levels are stored in thousands, so yoyAbs * 1000 is
    // the job count. Rounded to the nearest hundred deliberately: metro CES is
    // the national establishment survey sliced down to one city, and a tighter
    // figure would imply precision the sample does not carry.
    //
    // Twelve months rather than month-over-month for the same reason — a single
    // metro's monthly change often sits inside its own margin of error, so a
    // MoM count would render sampling noise as a signal. It is also how the
    // industry actually talks: "Phoenix added 41,000 jobs last year", never
    // "Phoenix added 300 jobs in March".
    const netJobs = s.employment.yoyAbs != null
      ? Math.round((s.employment.yoyAbs * 1000) / 100) * 100
      : null;
    const jobsStr = netJobs != null
      ? `${netJobs >= 0 ? "+" : "−"}${Math.abs(netJobs).toLocaleString()}`
      : null;
    rows.push({
      name: "Local Job Growth", cat: "DEMAND", type: "LEADING",
      value: jobsStr != null ? jobsStr : (yp != null ? pctStr(yp) : `${Math.round(s.employment.value)}K`),
      unit: jobsStr != null ? "jobs, past 12 mo" : (yp != null ? "jobs YoY" : "total (K)"),
      tone: netJobs != null ? (netJobs >= 0 ? "bull" : "bear") : (yp != null ? (yp >= 0 ? "bull" : "bear") : "neutral"),
      ...
      measures: "Total nonfarm employment for the metro (CBSA), shown as jobs added over the past 12 months. The clearest leading read on local rental demand.",
      impact: "Jobs move first, and the count is what you underwrite against: a market adding 40,000 jobs absorbs new supply that a market adding 4,000 cannot, even when both post the same growth rate. Accelerating metro employment precedes absorption and rent growth; a rollover is your earliest warning.",
    });
  }
