...
export function hellodataRows(hd) {
  ...
  if (eff) {
    ...
    rows.push({
      ...
      impact:
        "This is the number that underwrites a deal. Asking rent is what a property hopes for; effective rent is what shows up in the rent roll. When the two diverge, every valuation built on asking rent is overstated.",
    });
  }
  ...
  if (dom) {
    ...
    rows.push({
      ...
      measures:
        "Average days a listed unit sits before leasing, across 50+ unit multifamily properties only. Distinct from the Apartment List days-on-market row above, which covers a broader rental universe \u2014 the two are shown separately rather than blended.",
      impact:
