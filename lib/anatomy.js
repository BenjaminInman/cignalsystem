...
export const ANATOMY = [
  // ---- FLAGSHIP BASKET ----------------------------------------------------
  {
    ...
    components: [
      ...
      { label: "Core services, ex-shelter", role: "the balance · sticky", balance: true, kind: "index", cls: "coincident", weight: 26.1 },
      { label: "Core goods", role: "vehicles, apparel, furnishings", slug: "cpi_core_goods", kind: "index", cls: "leading", weight: 18.7 },
      { label: "Food & beverages", role: "at-home + away", slug: "cpi_food", kind: "index", cls: "coincident", weight: 13.4 },
      { label: "Energy", role: "gasoline + utilities", slug: "cpi_energy", kind: "index", cls: "leading", weight: 6.9 },
    ],
  },
  ...
  // ---- SENTIMENT DECOMPOSITION (live) -------------------------------------
  //
  // Deliberately a family, not a weighted_sum. UMich does publish a formula for
  // the headline, but it is computed from the underlying question responses and
  // then rebased and seasonally adjusted — it is NOT a blend of the two
  // sub-indices. Checked against 1978–2026: a simple average of ICC and ICE
  // misses the published headline by up to 4.55 points. Presenting it as a
  // basket with weights would be false precision, so the two legs are shown
  // side by side and the headline is displayed, not reconstructed.
  //
  // The headline leg reads umich_sentiment_direct rather than the platform
  // primary `consumer_sentiment` (FRED UMCSENT). That is the whole reason the
  // slug exists: UMich has FRED republish the headline a month late, so pairing
  // the FRED leg with same-day components would open a one-month gap inside a
  // single card. All three legs here come from one UMich vintage.
  //
  // Licensed display. Citation is exact and required, and is NOT the FRED
  // string — see the citation rendered on the card in the Anatomy page.
  {
    slug: "consumer_sentiment_anatomy",
    name: "Consumer Sentiment",
    type: "family",
    parentClass: "leading",
    // kind "level" (not "index") so the API returns the actual index reading
    // alongside the YoY percent. For sentiment the LEVEL is the story — the gap
    // between the two legs is what the card exists to show, and a pair of
    // percent changes hides it completely.
    headline: { slug: "umich_sentiment_direct", kind: "level", label: "headline sentiment" },
    formula: "Current Conditions + Expectations → published headline (not a weighted sum)",
    citation: "University of Michigan, Survey Research Center, Surveys of Consumers",
    teach:
      "One number, two very different clocks. Current Conditions asks how things are right now — it is coincident, and it mostly confirms what you already see in the market. Expectations asks where things are going, and it is genuinely leading; it is an official component of the US Index of Leading Economic Indicators. The headline blends them, which is why the headline alone is a weak signal: it can hold steady while the two legs move in opposite directions.\n\nThe gap is the read. Since 1978, Expectations has run about 18 points BELOW Current Conditions on average — consumers are structurally more pessimistic about the future than about the present. When that gap narrows or flips positive, it means people think the worst is behind them even though conditions have not improved yet. That has happened in only 16 of 583 months.\n\nFor multifamily: Expectations turning up ahead of Current Conditions is an early recovery tell, because leasing decisions are forward-looking — households form new ones when they believe their income is safe, not when the data confirms it. Expectations rolling over while Current Conditions holds is the opposite warning, and it typically shows in absorption before it shows in vacancy.",
    components: [
      { label: "Current Conditions", role: "how things are now · confirms", slug: "umich_current_conditions", kind: "level", cls: "coincident" },
      { label: "Expectations", role: "where things are going · LEI component", slug: "umich_expectations", kind: "level", cls: "leading" },
    ],
  },
